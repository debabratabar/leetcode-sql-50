...
select u.name , mr.user_id , count(mr.movie_id ) as count
from users u left join movierating mr on u.user_id = mr.user_id 
group by 1 , 2 )
order by count desc , name limit 1 )
union all
(
select  title as results from (
select m.title , mr.movie_id , avg(mr.rating) as avg_rating
