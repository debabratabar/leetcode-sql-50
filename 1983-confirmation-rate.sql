...
select user_id ,
count(user_id) total_action_per_user from Confirmations 
group by user_id 
) , 
...
temp_rate as (
    ...
    from total_counts tc left join total_confirmed_count tcc on tc.user_id = tcc.user_id 
)

select sn.user_id , round(case when tr.user_id is null then 0 else confirmation_rate end , 2 ) as confirmation_rate 
from signups sn left join  temp_rate tr on sn.user_id = tr.user_id   ; 